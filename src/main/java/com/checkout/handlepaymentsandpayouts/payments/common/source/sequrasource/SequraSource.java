package com.checkout.handlepaymentsandpayouts.payments.common.source.sequrasource;

import com.checkout.handlepaymentsandpayouts.payments.common.source.AbstractSource;
import com.checkout.handlepaymentsandpayouts.payments.common.source.SourceType;
import lombok.EqualsAndHashCode;
import lombok.Getter;
import lombok.Setter;
import lombok.ToString;

/**
 * sequra source Class
 * The source of the payment
 */
@Getter
@Setter
@EqualsAndHashCode(callSuper = true)
@ToString(callSuper = true)
public final class SequraSource extends AbstractSource {

    /**
     * Initializes a new instance of the SequraSource class.
     */
    public SequraSource() {
        super(SourceType.SEQURA);
    }

}
