package com.checkout.handlepaymentsandpayouts.payments.common.source.alipayplussource;

import com.checkout.handlepaymentsandpayouts.payments.common.source.AbstractSource;
import com.checkout.handlepaymentsandpayouts.payments.common.source.SourceType;
import lombok.EqualsAndHashCode;
import lombok.Getter;
import lombok.Setter;
import lombok.ToString;

/**
 * alipay_plus source Class
 * The source of the payment
 */
@Getter
@Setter
@EqualsAndHashCode(callSuper = true)
@ToString(callSuper = true)
public final class AlipayPlusSource extends AbstractSource {

    /**
     * Initializes a new instance of the AlipayPlusSource class.
     */
    public AlipayPlusSource() {
        super(SourceType.ALIPAY_PLUS);
    }

}
