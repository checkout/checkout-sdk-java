package com.checkout.handlepaymentsandpayouts.payments.common.source.benefitsource;

import com.checkout.handlepaymentsandpayouts.payments.common.source.AbstractSource;
import com.checkout.handlepaymentsandpayouts.payments.common.source.SourceType;
import lombok.EqualsAndHashCode;
import lombok.Getter;
import lombok.Setter;
import lombok.ToString;

/**
 * benefit source Class
 * The source of the payment
 */
@Getter
@Setter
@EqualsAndHashCode(callSuper = true)
@ToString(callSuper = true)
public final class BenefitSource extends AbstractSource {

    /**
     * Initializes a new instance of the BenefitSource class.
     */
    public BenefitSource() {
        super(SourceType.BENEFIT);
    }

}
